...
test_succeeds("sts_build_factored_variational_loss works", {
  ...
  if (tf$executing_eagerly()) {
    ...
    for (step in 1:5) {
      ...
      avg_loss <- Map(function(x) build_variational_loss() %>% as.numeric(), 1:3) %>% unlist() %>% mean()

      variational_distributions <- (observed_time_series %>% sts_build_factored_variational_loss(model = model))[[2]]
      posterior_samples <-
        Map(function(d)
          d %>% tfd_sample(50), variational_distributions %>% reticulate::py_to_r() %>% unname())
    }

  } else {

    loss_and_dists <- observed_time_series %>% sts_build_factored_variational_loss(model = model)
    ...
    with (tf$Session() %as% sess,  {
      ...
      avg_loss <- Map(function(x) sess$run(variational_loss), 1:2) %>% unlist() %>% mean()
