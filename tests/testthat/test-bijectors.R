...
test_succeeds("Define a scale_tril bijector", {
  b <- bijector_scale_tril(bijector_exp(), NULL)
  x <- c(0,0,0)
...
test_succeeds("Define a sinh_arcsinh bijector", {
  b <- bijector_sinh_arcsinh()
  x <- c(0, 1, 2)
  expect_equal(b %>% forward(x), bijector_identity() %>% forward(x))
})

test_succeeds("Define a softmax_centered bijector", {
  b <- bijector_softmax_centered()
  x <- tf$log(c(2, 3, 4))
  y <- c(0.2, 0.3, 0.4, 0.1)
...
test_succeeds("Define a permute bijector", {
  b <- bijector_permute(permutation = list(1, 3, 0, 2))
  x <- seq(0, 1, by = 0.3)
  y <- b %>% forward(x)
  expect_equivalent(y %>% tensor_value(), c(0.3, 0.9, 0, 0.6), tol = 1e6)

  b <- bijector_permute(permutation = list(2, 1, 0), axis = -2)
...
test_succeeds("Define a power transform bijector", {
  power <- 2
  b <- bijector_power_transform(power = power)
  x <- c(1, 2, 3)
  y <- b %>% forward(x)
...
test_succeeds("Define a matrix_inverse_tril bijector", {
  b <- bijector_matrix_inverse_tril()
  x <- matrix(c(1, 0, 2, 1), ncol = 2, byrow = TRUE)
...
test_succeeds("Define a matvec_lu bijector", {
  ...
  conv1x1 <- bijector_matvec_lu(fact[[1]],
                                fact[[2]],
                                validate_args = TRUE)
...
#test_succeeds("Define a reshape bijector", {
  #### this is tbd
  ####b <- bijector_reshape(event_shape_out = c(1, -1))
  # x <- c(1, 2, 3)
  # y <- b %>% forward(x)
  # expect_equivalent(b %>% inverse(y) %>% tensor_value(), x)
#})
...
test_succeeds("Use masked autoregressive flow with template", {
  ...
  target_dist <- distribution_normal(loc = 2.2, scale = 0.23)
